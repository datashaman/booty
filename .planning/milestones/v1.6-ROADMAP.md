...
Memory Agent converts high-value system events into durable, queryable knowledge and surfaces that knowledge back into the workflow as context. Informational only — no outcomes are blocked or altered.

## Phases

### Phase 22: Memory Foundation ✓ (planned)
...
**Plans:** 3 plans
- [ ] 22-01-PLAN.md — Config (MemoryConfig, .booty.yml memory block, env overrides)
- [ ] 22-02-PLAN.md — Store (memory.jsonl, append/read, schema)
- [ ] 22-03-PLAN.md — add_record API + dedup

---

### Phase 23: Ingestion ✓ (planned)
...
**Success Criteria:**
1. Observability issue-from-Sentry → incident record stored
2. Governor HOLD → governor_hold; deploy failure → deploy_failure
3. Security FAIL → security_block; ESCALATE → security_block with reason
4. Verifier FAIL (import/compile/test) → verifier_cluster
5. Revert commits on main → revert record

**Plans:** 4 plans
- [ ] 23-01-PLAN.md — Memory adapters (build_*_record for all 6 types)
- [ ] 23-02-PLAN.md — Webhooks: Observability, Governor HOLD, Deploy failure
- [ ] 23-03-PLAN.md — Security + Verifier wiring
- [ ] 23-04-PLAN.md — Revert: CLI + push handler

---

### Phase 24: Lookup ✓ (planned)
...
**Plans:** 1 plan
- [ ] 24-01-PLAN.md — Lookup query engine (path/fingerprint match, sort, limit, query API)

---

### Phase 25: Surfacing ✓ (planned)
...
2. Governor HOLD details include 1–2 memory links when relevant
...
**Plans:** 1 plan
- [ ] 26-01-PLAN.md — booty memory status + query (MEM-27, MEM-28)
...
|-------|------|-------|--------|
| 22. Memory Foundation | Storage, config, API | 3 | Not started |
| 23. Ingestion | Wire adapters | 4 | Not started |
